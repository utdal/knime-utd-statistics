...
def run_bonferroni_test(data, groups, alpha=0.05):
    # High-level: perform pairwise Welch t-tests across groups, collect raw
    # statistics, then apply Holm (sequential) correction to obtain
    # corrected p-values and boolean reject decisions. Returns results and
    # a group-summary for downstream formatting.
    ...
    rejected, corrected_pvalues, alpha_sidak, alpha_bonf = multipletests(raw_pvalues, alpha=alpha, method="holm")
...
def format_bonferroni_results_for_knime(bonferroni_output):
    ...
    # High-level: convert the raw results dict from run_bonferroni_test into
    # (pairwise_df, group_summary_df) that match the KNIME schema and types.
    # This sets explicit 'P-Value' (raw), 'Corrected P-Value', and metadata.
    # Format pairwise comparisons table
    pairwise_df = bonferroni_output["results"].copy()

    # Keep raw P-Value separate and populate Corrected P-Value column
    # 'P-Value' should represent the raw (uncorrected) p-value; 'Corrected P-Value' holds the adjusted value
    pairwise_df["P-Value"] = pairwise_df["Raw P-Value"]
    ...
    # Populate significance level in group summary so consumers know the alpha used
    group_summary_df["Significance Level"] = bonferroni_output.get("alpha", np.nan)
