...
def validate_anova_data(data, groups):
    ...
    if n_groups < 2:
        raise ValueError(
            f"Found only {n_groups} group(s). ANOVA requires at least 2 groups for comparison."
        )
    ...
    if min_n < 2:
        ...
        raise ValueError(
            ...
            f"Each group must have at least 2 observations for ANOVA."
        )
    ...
    if zero_var_groups:
        raise ValueError(
            ...
            f"ANOVA requires within-group variation."
        )
