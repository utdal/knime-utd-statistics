...
class TestType(knext.EnumParameterOptions):
    ...
    BREUSCH_PAGAN = (
        "Breusch-Pagan",
        "Tests if error variance is related to the predictor variables. "
        "Most commonly used test, assumes normally distributed errors."
    )
    WHITE = (
        "White",
        "General test that doesn't assume normal distribution. "
        "Detects more complex patterns but is computationally intensive."
    )
    GOLDFELD_QUANDT = (
        "Goldfeld-Quandt",
        "Compares variance between two subgroups of data. "
        "Requires specifying a sort variable and split fraction."
    )
...
target_column_param = knext.ColumnParameter(
    ...
    description=(
        ...
        "This is the dependent variable in your regression model."
    ),
