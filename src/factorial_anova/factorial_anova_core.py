...
def prepare_anova_data(
    ...
    if missing_count > 0:
        ...
        if dropped_count > 0:
            warnings.append(
                f"Dropped {dropped_count} rows containing missing values "
    ...
    if len(work_df) < 3:
        raise ValueError(
            f"Insufficient data for ANOVA. Only {len(work_df)} complete rows available. "
    ...
    if not pd.api.types.is_numeric_dtype(work_df[response_col]):
        raise ValueError(
            f"Response column '{response_col}' must be numeric. "
            f"Found type: {work_df[response_col].dtype}"
        )
    ...
    for col in factor_cols:
        ...
        if n_unique < 2:
            raise ValueError(
                f"Factor '{col}' has only {n_unique} level(s). "
                "Each factor must have at least 2 levels for ANOVA."
            )
...
def run_factorial_anova(
    ...
    formula = build_anova_formula(
        response_col, factor_cols, include_interactions, max_interaction_order
    )
    ...
    if n_obs < n_params + 20:
        warnings.append(
            f"⚠️ Small sample size: n={n_obs} observations with {n_params} parameters. "
            f"Recommended: n > {n_params + 20}. Results may be unreliable."
        )
    
    # Step 10: High-order interaction warning
    if include_interactions and max_interaction_order >= 3 and len(factor_cols) >= 3:
        warnings.append(
            "⚠️ High-order interactions (3-way+) can produce large, "
